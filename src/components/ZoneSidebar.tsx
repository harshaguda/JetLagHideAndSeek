...
async function selectionProcess(
    ...
    for (const question of questions.get()) {
        ...
        if (
            ...
            if (question.id === "matching" && (question.data as any).showPlaces) {
                poiOverlays.push({
                    questionKey: question.key,
                    typeLabel: question.data.type,
                    candidates: nearestPoints,
                    nearest: nearestQuestion,
                });
            }
    ...
    // Overlay any requested POI markers after showGeoJSON (so they don't get removed
    // by removeHidingZones inside showGeoJSON).
    if (poiOverlays.length > 0) {
        // Remove any existing POI overlay layers from previous selections.
        map.eachLayer((layer: any) => {
        ...
        for (const overlay of poiOverlays) {
            const candidatesLayer = L.geoJSON(
                turf.featureCollection(overlay.candidates) as any,
                {
                    ...
                    onEachFeature: (feature: any, layer) => {
                        const name =
                            ...
                            "Unnamed";
                        layer.bindPopup(
                            `<b>${name}</b><br/>Type: ${overlay.typeLabel}`,
                        );
                    },
                },
            );
            ...
            const nearestLayer = L.geoJSON(overlay.nearest as any, {
                pointToLayer: (_feature, latlng) =>
                    L.circleMarker(latlng, {
                        radius: 7,
                        color: "green",
                ...
                onEachFeature: (feature: any, layer) => {
                    const name =
                        ...
                        "Unnamed";
                    layer.bindPopup(
                        `<b>Nearest to question:</b> ${name}<br/>Type: ${overlay.typeLabel}`,
                    );
                },
            });
            // @ts-expect-error intentionally mark custom layer
            nearestLayer.matchingPoiOverlay = true;
            nearestLayer.addTo(map);
        }
    } else {
        // No overlays requested; clear any existing POI overlay layers.
        map.eachLayer((layer: any) => {
            if (layer.matchingPoiOverlay) {
                map.removeLayer(layer);
            }
